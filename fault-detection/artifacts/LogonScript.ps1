...
$extensionExists = az k8s-extension show --resource-group "$env:resourceGroup" --cluster-name "$env:arcClusterName" --cluster-type connectedClusters --name azure-arc-containerstorage --query "extensionType" --output tsv
if ($extensionExists -eq "microsoft.arc.containerstorage") {
   ...
   az k8s-extension create --resource-group "$env:resourceGroup" --cluster-name "$env:arcClusterName" --cluster-type connectedClusters --name azure-arc-containerstorage --extension-type microsoft.arc.containerstorage --config-file "config.json"
   Write-Host "Edge Storage Accelerator Arc Extension installed successfully."
}

#

$extId = az k8s-extension list --cluster-name "$env:arcClusterName" --resource-group $env:resourceGroup --cluster-type connectedClusters | jq --arg extType microsoft.arc.containerstorage 'map(select(.extensionType == microsoft.arc.containerstorage)) | .[] | .identity.principalId' -r
